...
<template>
  <div class="bg-gray-900 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-4xl bg-gray-800 p-8 rounded-lg shadow-lg">
      <h1 class="text-3xl font-bold text-center mb-2 text-white">LexTrack Daily Activities 📝</h1>
      <p class="text-center text-gray-400 mb-6">Select a date to log or view your activities.</p>

      <!-- Date Selector -->
      <div class="mb-8 max-w-sm mx-auto">
        <label for="date" class="block text-sm font-medium text-gray-300 mb-2">Selected Date</label>
        <DatePicker
            ...
            class="w-full"
            :pt="{
            input: { class: 'w-full bg-gray-700 border-gray-600 text-white rounded-md' },
      ...
      <TabView :pt="{
        ...
        <!-- Main Tasks Tab -->
        <TabPanel value="0" header="Main Tasks" :pt="{ headerAction: { class: 'bg-gray-800 text-white' } }">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Form for adding a new task -->
            <div class="space-y-4 p-4 bg-gray-700/50 rounded-lg">
              <h3 class="font-bold text-lg text-white">Add New Task</h3>
              <div>
                <InputText v-model="newTask.name" placeholder="Task Name" class="w-full bg-gray-700 text-white" />
              </div>
              <div>
                <InputText v-model="newTask.jira" placeholder="Jira Ticket Link" class="w-full bg-gray-700 text-white" />
              </div>
              <div>
                <Editor v-model="newTask.desc" editorStyle="height: 120px" :pt="editorStyle" />
              </div>
              <Button label="Add Task" @click="addTask" class="w-full bg-indigo-600 hover:bg-indigo-700" />
            </div>
            <!-- List of existing tasks -->
            <div class="space-y-3 p-4 bg-gray-900/50 rounded-lg max-h-96 overflow-y-auto">
              <h3 class="font-bold text-lg text-white">Logged Tasks for Today</h3>
              <div v-if="!tasks.length" class="text-gray-400">No tasks logged for this date.</div>
        ...
        <!-- Meetings Tab -->
        <TabPanel value="1" header="Meetings" :pt="{ headerAction: { class: 'bg-gray-800 text-white' } }">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Form for adding a new meeting -->
            <div class="space-y-4 p-4 bg-gray-700/50 rounded-lg">
              <h3 class="font-bold text-lg text-white">Add New Meeting</h3>
              <div>
                <InputText v-model="newMeeting.title" placeholder="Meeting Title" class="w-full bg-gray-700 text-white" />
              </div>
              <div>
                <InputNumber v-model="newMeeting.duration" placeholder="Duration (minutes)" class="w-full" inputClass="bg-gray-700 text-white w-full rounded-md" />
              </div>
              <div>
                <Editor v-model="newMeeting.desc" editorStyle="height: 120px" :pt="editorStyle" />
              </div>
              <Button label="Add Meeting" @click="addMeeting" class="w-full bg-indigo-600 hover:bg-indigo-700" />
            </div>
            <!-- List of existing meetings -->
            <div class="space-y-3 p-4 bg-gray-900/50 rounded-lg max-h-96 overflow-y-auto">
              <h3 class="font-bold text-lg text-white">Logged Meetings for Today</h3>
              <div v-if="!meetings.length" class="text-gray-400">No meetings logged for this date.</div>
              <div v-for="(meeting, index) in meetings" :key="`meeting-${index}`" class="bg-gray-700 p-3 rounded-md">
        ...
        <!-- Admin Support Tab -->
        <TabPanel value="2" header="Admin Support" :pt="{ headerAction: { class: 'bg-gray-800 text-white' } }">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Form for adding new admin support -->
            <div class="space-y-4 p-4 bg-gray-700/50 rounded-lg">
              <h3 class="font-bold text-lg text-white">Add Admin/Support Task</h3>
              <div>
                <InputText v-model="newAdminSupport.title" placeholder="Task Title" class="w-full bg-gray-700 text-white" />
              </div>
              <div>
                <Editor v-model="newAdminSupport.desc" editorStyle="height: 160px" :pt="editorStyle" />
              </div>
              <Button label="Add Support Task" @click="addAdminSupport" class="w-full bg-indigo-600 hover:bg-indigo-700" />
            </div>
...
.p-datepicker-header, .p-tabview-nav-link {
...
.p-datepicker-title, .p-datepicker-next, .p-datepicker-prev {
  color: #f9fafb !important;
}
