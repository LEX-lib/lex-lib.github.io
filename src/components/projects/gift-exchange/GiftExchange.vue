...
const checkStatus = async () => {
    ...
    loading.value = true;
    try {
        const record = await pb.collection('gift_exchange_participants').getFirstListItem(`code="${userCode.value.toUpperCase()}"`);
        if (record) {
            currentUser.value = record;
            isEnrolled.value = true;
...
const generatePairs = async () => {
  ...
  try {
    ...
    if (currentUser.value) {
        const updated = await pb.collection('gift_exchange_participants').getOne(currentUser.value.id);
  ...
  <div class="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4 font-sans text-gray-900 perspective-1000">
    <div class="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden relative z-10">
      <div class="p-8 text-center">
        ...
        <p class="text-gray-500 mb-8">Join the celebration and exchange gifts with your friends!</p>
        ...
        <div v-if="!isEnrolled">
          <div class="space-y-6 text-left">
            ...
            <div class="bg-gray-50 p-4 rounded-xl border border-gray-200">
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">Enter Your Code</label>
                  <input v-model="userCode" type="text" placeholder="ABCD" maxlength="4" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-black focus:border-transparent outline-none transition-all uppercase tracking-widest text-center font-mono" />
                </div>
        ...
        <div v-else>
            ...
            <div v-if="generatedCode && !drawnName" class="py-6">
                ...
                <p class="text-sm text-gray-400">Waiting for the drawing event to start...</p>
                ...
                <div class="flex flex-col gap-2 mt-6">
                    ...
                    <button @click="isEnrolled = false; name = ''; generatedCode = ''; userCode = ''" class="text-sm text-gray-500 underline">
                        Back to Home
                    </button>
                </div>
            </div>
            ...
            <div v-else-if="drawnName" class="py-8">
                ...
                <button @click="isEnrolled = false; name = ''; generatedCode = ''; userCode = ''" class="mt-8 text-sm text-gray-500 underline">
                    Back to Home
                </button>
            </div>
        ...
        <div class="mt-8 border-t pt-6">
            ...
            <div v-if="isSuperUser">
                <button @click="isAdminOpen = !isAdminOpen; if(isAdminOpen) fetchParticipants()" class="mt-4 text-xs text-gray-300 hover:text-gray-500 transition-colors">
                    {{ isAdminOpen ? 'Hide Admin' : 'Show Admin' }}
                </button>
                
                <div v-if="isAdminOpen" class="mt-4 p-4 bg-gray-100 rounded-lg text-left">
                    <button @click="generatePairs" :disabled="loading" class="w-full bg-red-600 text-white font-bold py-2 px-4 rounded hover:bg-red-700 transition-colors text-sm mb-4">
                        ...
                        <table class="w-full">
                            ...
                            <tbody>
                                <tr v-for="p in participants" :key="p.id" class="border-b last:border-0">
                                    ...
                                    <td class="py-1 text-gray-600">{{ p.drawn_name || '-' }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
