...
const validateLobby = async () => {
  ...
  try {
    ...
    const now = new Date();
    const deadline = lobby.value.enrollment_deadline ? new Date(lobby.value.enrollment_deadline) : null;
...
const enroll = async () => {
  if (enrollmentClosed.value) {
    toast.error("Enrollment is closed. The drawing has already started or the deadline has passed.");
    return;
  }
  ...
  <div class="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4 font-sans text-gray-900">
    <div class="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden relative z-10">
      <div class="p-8 text-center">
        ...
        <h2 class="text-xl font-semibold text-gray-600 mb-6">Join the Exchange</h2>
        <p class="text-gray-500 mb-8">Sign up to participate in the gift giving!</p>

        <!-- Step 1: Enter Lobby Code -->
        <div v-if="!lobbyValidated" class="bg-gray-50 p-4 rounded-xl border border-gray-200 text-left">
            <h3 class="font-bold text-lg mb-2">Enter Lobby Code</h3>
            <p class="text-sm text-gray-600 mb-3">Ask your organizer for the lobby code</p>
            <div>
            ...
            <button @click="validateLobby" :disabled="loading" class="mt-4 w-full bg-black text-white font-bold py-3 px-4 rounded-lg hover:bg-gray-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <div v-else-if="enrollmentClosed" class="py-6">
            ...
            <p class="text-gray-600 mb-2">Sorry, enrollment has ended for <strong>{{ lobby.lobby_name }}</strong>.</p>
            <p class="text-sm text-gray-500 mb-6">The drawing has already started or the deadline has passed.</p>
            
            <div class="flex gap-2">
                <button @click="lobbyValidated = false; lobbyCode = ''" class="flex-1 text-sm text-gray-500 underline">
                    Try Different Lobby
                </button>
                <RouterLink to="/projects/gift-exchange/draw" class="flex-1 text-sm text-black font-semibold underline">
                    Draw Here
                </RouterLink>
            </div>
        </div>
        ...
        <div v-else-if="!isEnrolled">
            <div class="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <button @click="lobbyValidated = false; lobbyCode = ''" class="text-xs text-blue-600 underline mt-1">
                    Change Lobby
                </button>
            </div>
            ...
            <div class="bg-gray-50 p-4 rounded-xl border border-gray-200 text-left">
                ...
                <div>
                  ...
                  <input v-model="name" type="text" placeholder="Juan dela Cruz" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-black focus:border-transparent outline-none transition-all" />
                </div>
                <button @click="enroll" :disabled="loading" class="mt-4 w-full bg-black text-white font-bold py-3 px-4 rounded-lg hover:bg-gray-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                    {{ loading ? 'Processing...' : 'Enroll & Get Code' }}
                </button>
            </div>
            
            <div class="mt-6 text-sm text-gray-500">
                Already have a code? <RouterLink to="/projects/gift-exchange/draw" class="underline text-black font-semibold">Draw here</RouterLink>
            </div>
        </div>
        ...
        <div v-else class="py-6">
            ...
            <p class="text-gray-600 mb-4">Save this code. You will need it to draw your gift.</p>
            
            <div class="text-5xl font-mono font-bold tracking-widest text-black bg-yellow-100 p-4 rounded-xl border-2 border-yellow-300 mb-6 select-all">
                {{ generatedCode }}
            </div>

            <p class="text-sm text-gray-400 mb-6">Waiting for the drawing event to start...</p>
            
            <RouterLink to="/projects/gift-exchange/draw" class="block w-full bg-black text-white font-bold py-3 px-4 rounded-lg hover:bg-gray-800 transition-colors text-sm">
                Go to Draw Page
            </RouterLink>
        </div>

        <div v-if="lobbyValidated" class="mt-8 border-t pt-6">
            <p class="text-sm text-gray-500">{{ participantsCount }} participants enrolled in this lobby</p>
        </div>
