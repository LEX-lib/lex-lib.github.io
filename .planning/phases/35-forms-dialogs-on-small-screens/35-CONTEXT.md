...
- **D-35-13:** FD-04 DatePicker gets `:touchUI="isMobile"` at every site of use (ManageExpense date, ExpensesToolbar From/To, ExpensesReportsView Custom range, ManageVaccination date, ManageMembership expiry). Preserve the `administeredDate` direct-v-model binding (do NOT fold back into `@primevue/forms`).
