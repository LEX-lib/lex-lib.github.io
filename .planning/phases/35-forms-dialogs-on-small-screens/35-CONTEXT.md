...
- **D-35-13 (RE-CORRECTED 2026-05-28 after UAT):** FD-04 — **all DatePickers use the default tap-to-open popup overlay on mobile (NO `:inline`).** The 2026-05-27 inline approach was reverted during the 35-06 human-verify: `:inline="isMobile"` on the always-rendered Expenses filter toolbar (From/To) + Reports custom range produced two huge always-visible calendars that crowded the list before any data showed. Inline is wrong for persistent on-page pickers, and the user chose popup-everywhere over a dialog-only split. Usability on mobile is carried by the FD-01 16px no-zoom rule (the popup is tap-to-open and closes on select). `showButtonBar` (added with inline) was also removed. Preserve the `administeredDate` direct-v-model binding regardless (#8191). *(Superseded note, kept for history: PrimeVue 4.5.5 dropped the PrimeVue-3 Calendar `touchUI` prop — that's why inline was tried; inline then proved wrong for filter pickers.)*
