# Requirements Archive: Lexarium v4.0 Daily Expense Tracker

**Archived:** 2026-05-22 (milestone shipped)
**Original:** .planning/REQUIREMENTS.md (now deleted — see git history)
...
## v4.0 Requirements (All Delivered)

### Backend & Type Foundation

- [x] **EXP-01**: `wallecx_expenses` PocketBase collection with fields: `id` (auto), `user` (relation to users), `amount` (number, decimal, > 0), `expense_date` (date, required), `category` (text, references a name from `wallecx_expense_categories`), `description` (text, short, required), `notes` (text, longer, optional), `receipt` (file, single image, optional). 5 per-user access rules (list, view, create, update, delete) enforcing `@request.auth.id = user.id` server-side. — **Validated Phase 23 (2026-05-21)**
- [x] **EXP-02**: `wallecx_expense_categories` PocketBase collection with fields: `id`, `user` (relation), `name` (text, required, unique within user via compound index). DEFAULT_EXPENSE_CATEGORIES constant defined (Food, Transport, Bills, Health, Shopping, Entertainment, Other); lazy per-user seeding is Phase 24. 5 per-user rules. — **Validated Phase 23 (2026-05-21)**
- [x] **EXP-03**: TypeScript types in `src/types/wallecx/expenses/types.d.ts` (extending PocketBase `RecordModel`); Zod schema for write-path validation; `expenseMapper.ts` matching the existing `vaccinationMapper`/`membershipMapper` pattern (strip read-only fields on write, id-refresh contract). 9 Vitest tests pass. — **Validated Phase 23 (2026-05-21)**

### Write Path — Tab Shell + CRUD

- [x] **EXP-04**: `WallecxApp.vue` updated to render a third tab "Expenses" after the existing Memberships tab. Tab order: Vaccinations / Memberships / Expenses. — **Validated Phase 24 (2026-05-21)**
- [x] **EXP-05**: `ExpensesTab.vue` and `ManageExpense.vue` CRUD dialog. Manage dialog matches the existing Wallecx pattern: Zod `safeParse`, `isSaving` guard preventing double-submit, server-first delete via `useConfirm`, EXIF-stripped receipt photo upload, conditional file field, direct `v-model` refs (no `@primevue/forms` due to PrimeVue ColorPicker #8135 lessons). — **Validated Phase 24 (2026-05-21)**
- [x] **EXP-06**: User can add a custom category from within the manage flow (e.g., inline "Add new category…" option in the category picker). New category is persisted to `wallecx_expense_categories` for that user only. — **Validated Phase 24 (2026-05-21)**

### Read Path — List View

- [x] **EXP-07**: `ExpensesTab.vue` list view shows all of the authenticated user's expenses, sortable by date (newest/oldest), category (A–Z), and amount (high/low). Sort mode persists in sessionStorage under `wallecx:expense-sort` (mirrors view-toggle session pattern from Phase 8). — **Validated Phase 25 (2026-05-21)**
- [x] **EXP-08**: Filter by category (multi-select chip pill or dropdown) and by date range (start + end). Filters are client-side computed; no new PocketBase queries. — **Validated Phase 25 (2026-05-21)**
- [x] **EXP-09**: Client-side search by description text (reactive instant filtering — no debounce, matches MembershipsTab/VaccinationsTab since `@vueuse/core` is not installed). — **Validated Phase 25 (2026-05-21)**
- [x] **EXP-10**: Receipt photo preview when available via the existing `AttachmentPreview` component (image/PDF/download branching with short-lived view-time tokens). — **Validated Phase 25 (2026-05-21)**

### Reporting View

- [x] **EXP-11**: Period-tabbed reporting view inside `ExpensesTab.vue` as a second sub-tab (List / Reports). Period tabs: This Month / This Quarter / This Year / Custom range (with start/end date pickers). — **Validated Phase 26 (2026-05-22)**
- [x] **EXP-12**: Each period view shows the grand total spend for that period. — **Validated Phase 26 (2026-05-22)**
- [x] **EXP-13**: Each period view shows a per-category breakdown chart (horizontal bar) using PrimeVue Chart (Chart.js wrapper). chart.js@^4.5.1 installed. — **Validated Phase 26 (2026-05-22)**
...
| EXP-04 | Phase 24 | Complete (2026-05-21) |
| EXP-05 | Phase 24 | Complete (2026-05-21) |
| EXP-06 | Phase 24 | Complete (2026-05-21) |
...
| EXP-11 | Phase 26 | Complete (2026-05-22) |
| EXP-12 | Phase 26 | Complete (2026-05-22) |
| EXP-13 | Phase 26 | Complete (2026-05-22) |

**Coverage:** 13/13 requirements — 100% delivered

---

*Requirements archived: 2026-05-22 at v4.0 milestone close*
*All 13 EXP requirements shipped across Phases 23–26*
