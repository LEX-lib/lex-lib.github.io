...
**Goal:** All 4 Manage dialogs (`ManageExpense`, `ManageBudget`, `ManageMembership`, `ManageVaccination`) render through a shared `BaseMobileDialog.vue` wrapper with sticky bottom action bars that stay above the virtual keyboard, iOS auto-zoom-on-focus is eliminated via a global ≥16px `.p-inputtext` rule, every input carries appropriate `inputmode` / `autocomplete` / `enterkeyhint`, DatePicker uses inline mode (`:inline="isMobile"`) on mobile (touchUI dropped in PrimeVue 4.5.5 — see D-35-13), FileUpload opens the device camera via `capture="environment"` with a gallery-picker fallback, focused inputs auto-scroll into view when the keyboard opens, and Drawer dismissal on a dirty form gates discard via `useConfirm`. Per-dialog migration order: ManageExpense → ManageBudget → ManageMembership → ManageVaccination (ColorPicker direct-v-model invariant preserved; PrimeVue #8135 workaround intact).
...
4. DatePicker on every site of use (`ManageExpense` date, `ExpensesToolbar` From/To, `ExpensesReportsView` Custom range, `ManageVaccination` date, `ManageMembership` expiry) renders as a full-screen inline calendar (`:inline="isMobile"`) on mobile; native popup behavior remains on desktop
